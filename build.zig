...
const constants = @import("src/constants.zig").Constants;
...
pub fn build(b: *std.Build) !void {
    ...
    build_options.addOption([:0]const u8, "app_id", constants.app_id);
    ...
    const exe = b.addExecutable(.{
        ...
        .root_module = mod,
        .use_llvm = true, // needed for linking with lib build with gcc 16.x.x
    });
    ...
    const metainfo_install = b.addInstallFileWithDir(metainfo, metainfo_dir, b.fmt("{s}.metainfo.xml", .{constants.app_id}));
    b.getInstallStep().dependOn(&metainfo_install.step);

    // desktop file install
    const desktop = generateDesktop(b, linguas);
    const desktop_install = b.addInstallFileWithDir(desktop, desktop_dir, b.fmt("{s}.desktop", .{constants.app_id}));
    b.getInstallStep().dependOn(&desktop_install.step);

    // install app icon
    const scalable_icon = if (devel)
        b.path(b.fmt("data/icons/hicolor/scalable/apps/{s}.Devel.svg", .{constants.app_id}))
    else
        b.path(b.fmt("data/icons/hicolor/scalable/apps/{s}.svg", .{constants.app_id}));
    const scalable_icon_install = b.addInstallFileWithDir(scalable_icon, icon_dir, b.fmt("hicolor/scalable/apps/{s}.svg", .{constants.app_id}));
    b.getInstallStep().dependOn(&scalable_icon_install.step);

    const symbolic_icon = b.path(b.fmt("data/icons/hicolor/symbolic/apps/{s}-symbolic.svg", .{constants.app_id}));
    const symbolic_icon_install = b.addInstallFileWithDir(symbolic_icon, icon_dir, b.fmt("hicolor/symbolic/apps/{s}-symbolic.svg", .{constants.app_id}));
...
fn generateMetainfo(b: *std.Build, linguas: []const []const u8) std.Build.LazyPath {
    const base = b.path(b.fmt("data/{s}.metainfo.xml",.{constants.app_id}));
    ...
    return translate.addPrefixedOutputFileArg("-o", b.fmt("data/{s}.metainfo.xml",.{constants.app_id}));
}

fn generateDesktop(b: *std.Build, linguas: []const []const u8) std.Build.LazyPath {
    const base = b.path(b.fmt("data/{s}.desktop",.{constants.app_id}));

    const translate = b.addSystemCommand(&.{ "msgfmt", "--desktop" });
    addPoDependencies(b, translate, linguas);
    translate.addPrefixedDirectoryArg("-d", b.path("po"));
    translate.addPrefixedFileArg("--template=", base);
    return translate.addPrefixedOutputFileArg("-o", b.fmt("data/{s}.desktop",.{constants.app_id}));
}
